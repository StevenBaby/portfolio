<template>
  <div class="page-container">
    <div class="stats-bar">
      <div class="stat-card">
        ...
        <div class="stat-value">
          {{ displayData(latest?.total, hideAmount, money, "***") }}
        </div>
      </div>
      <div class="stat-card">
        <div class="stat-label">总资产增量</div>
        <div
          class="stat-value"
          :class="totalDelta >= 0 ? 'amount-positive' : 'amount-negative'"
        >
          {{ totalDeltaText }}
        </div>
      </div>
      <div class="stat-card">
        <div class="stat-label">资产类别</div>
        <div class="stat-value">{{ visibleAssets.length }}</div>
      </div>
      <div class="stat-card">
    ...
    <div class="filter-bar">
      <n-checkbox
        v-model:checked="showPension"
        class="ml-auto"
        :disabled="editPropertyInfo"
        >公积金</n-checkbox
      >
    ...
    <div v-show="subTab === 'summary'" class="property-summary-actions">
      <template v-if="editPropertyInfo">
        <n-input
          v-model:value="addingAsset"
          size="small"
          placeholder="新增资产类型"
          style="width: 160px"
          @keyup.enter="addAsset"
        />
        <n-button size="small" type="primary" @click="addAsset"
          >添加品种</n-button
        >
        <n-date-picker
        ...
        <n-button
          ...
          size="small"
          type="primary"
          @click="saveSnapshotEdit"
          >保存快照</n-button
        >
        <n-button
          v-if="editingSnapshot"
          size="small"
          @click="cancelSnapshotEdit"
          >取消</n-button
        >
        <n-button v-else size="small" @click="startSnapshotEdit"
          >编辑最新快照</n-button
        >
      </template>
    </div>
    ...
    <div
      v-show="subTab === 'pie'"
      class="chart-container property-chart-container"
    >
      <v-chart
        :key="chartRenderKey"
        :option="pieOption"
...
const chartRenderKey = computed(
  () => `${hideAmount.value}-${chartMetric.value}-${showPension.value}`,
);
const visibleAssets = computed(() =>
  showPension.value
    ? assets.value
    : assets.value.filter((asset) => asset !== "公积金"),
);
const visibleRows = computed(() =>
  rows.value.map((row) => {
    const values = Object.fromEntries(
      visibleAssets.value.map((asset) => [asset, row.values[asset] || 0]),
    );
    return {
      ...row,
      values,
...
const totalDelta = computed(() =>
  latest.value && previous.value
    ? latest.value.total - previous.value.total
    : 0,
);
...
function assetColor(asset, index = 0) {
  return (
    propertyProfiles.value[asset]?.color ||
    presetColors[index % presetColors.length]
  );
}
function valueText(value) {
  return displayData(value, hideAmount.value, money, "***");
}
function percentText(value) {
  return displayData(value, hideAmount.value, (v) => `${v.toFixed(2)}%`, "***");
}
function updateAssetColor(row, color) {
  propertyProfiles.value = {
    ...propertyProfiles.value,
    [row.asset]: { color },
  };
...
async function saveSnapshotEdit() {
  ...
  const target = nextRows.find((row) => row.date === date) || {
    date,
    values: {},
  };
  target.date = date;
  target.values = Object.fromEntries(
    nextAssets.map((asset) => [asset, editingValues.value[asset] ?? 0]),
  );
  if (!nextRows.some((row) => row.date === date)) nextRows.push(target);
  const response = await fetch(`${API}/api/properties`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ assets: nextAssets, rows: nextRows }),
  });
...
const summaryColumns = computed(() => [
  ...
  },
  {
    ...
    render: (row) =>
      editingSnapshot.value
        ? h(NInput, {
            value: String(editingValues.value[row.asset] ?? 0),
            size: "small",
            type: "text",
            onUpdateValue: (value) => {
              editingValues.value = {
        ...
        : h("span", { class: "amount-positive" }, valueText(row.value)),
  ...
  },
  {
    ...
    render: (row) => {
      ...
      return h(
        "span",
        { class: delta >= 0 ? "amount-positive" : "amount-negative" },
        displayData(delta, hideAmount.value, money, "***"),
      );
    },
    sorter: (a, b) => a.value - b.value,
  },
  {
    ...
    key: "percent",
    align: "right",
    render: (row) => percentText(row.percent),
    sorter: (a, b) => a.percent - b.percent,
  },
  ...(editPropertyInfo.value
    ? [
        {
          title: "颜色",
          key: "color",
          ...
          render: (row) =>
            hideAmount.value
              ...
              : h(NColorPicker, {
                  ...
                  showAlpha: false,
                  size: "small",
                  onUpdateValue: (value) => updateAssetColor(row, value),
                  "onUpdate:value": (value) => updateAssetColor(row, value),
                }),
        },
      ]
    : []),
...
const columns = computed(() => [
  ...
  ...visibleAssets.value.map((asset, index) => ({
    title: asset,
    key: asset,
    align: "right",
    render: (row) =>
      h(
        "span",
        { style: { color: assetColor(asset, index) } },
        valueText(row.values[asset]),
      ),
  })),
  {
    title: "总资产",
    ...
    align: "right",
    render: (row) =>
      h("span", { class: "amount-positive" }, valueText(row.total)),
  },
]);
    ...
    values: Object.fromEntries(
      visibleAssets.value.map((asset) => [
        ...
        chartMetric.value === "delta" && index > 0
          ? (row.values[asset] || 0) -
            (chronological[index - 1].values[asset] || 0)
          : chartMetric.value === "delta"
            ? 0
            : row.values[asset] || 0,
      ]),
    ),
...
const latestValues = computed(() =>
  visibleAssets.value.map((asset) => latest.value?.values?.[asset] || 0),
);
...
const barOption = computed(() => ({
  ...
  yAxis: {
    type: "value",
    axisLabel: { color: "#888", formatter: (value) => axisValue(value) },
    splitLine: { lineStyle: { color: "#222" } },
  },
  series: visibleAssets.value.map((asset, index) => ({
    name: asset,
    type: "bar",
    stack: "asset-total",
    data: chartRows.value.map((row) => row.values[asset] || 0),
    itemStyle: { color: assetColor(asset, index) },
    barMaxWidth: 42,
  })),
}));
const pieOption = computed(() => ({
  ...
  tooltip: {
    trigger: "item",
    formatter: (params) =>
      `${hideAmount.value ? "***" : params.name}<br/>${axisValue(
        params.value,
      )} (${params.percent}%)`,
  },
  legend: {
  ...
  series: [
    {
      ...
      radius: "60%",
      center: ["50%", "45%"],
      data: latestValues.value
        .map((value, index) => ({
          name: visibleAssets.value[index],
          value,
          itemStyle: { color: assetColor(visibleAssets.value[index], index) },
        }))
...
const lineOption = computed(() => ({
  ...
  series: visibleAssets.value.map((asset, index) => ({
    ...
    data: chartRows.value.map((row) => row.values[asset] || 0),
    itemStyle: { color: assetColor(asset, index) },
    lineStyle: { color: assetColor(asset, index) },
  })),
}));
function selectLegend() {
  legendSelected.value = Object.fromEntries(
    visibleAssets.value.map((asset) => [asset, true]),
  );
}
function clearLegend() {
  legendSelected.value = Object.fromEntries(
    visibleAssets.value.map((asset) => [asset, false]),
  );
}
